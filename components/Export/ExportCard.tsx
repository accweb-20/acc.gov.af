...
export default function ExportCard({ item }: { item: ExportItem }) {
  const CARD_MAX = 360;
  const OVERLAP = 26;
  ...
  return (
    <article aria-labelledby={`export-${item._key ?? slug}-title`} className="flex justify-center w-full text-[#1A1A1A]">
      <div
        ref={wrapperRef}
      ...
      >
        ...
        >
          <div>
            <h3
              id={`export-${item._key ?? slug}-title`}
          ...
          <div className="w-full flex items-center justify-between" style={{ gap: 12 }}>
            <a
              ...
              aria-label={`Read more about ${item.title ?? "export item"}`}
              style={{
                padding: "8px 14px",
