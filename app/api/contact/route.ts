...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  ...
  console.error(
    "Missing Supabase server env vars for /api/contact. Check SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY."
  );
}
...
export async function POST(req: Request) {
  try {
    ...
    const first_name = isString(body.first_name)
      ? body.first_name.trim()
      : "";

    const last_name = isString(body.last_name)
      ? body.last_name.trim()
      : "";
    ...
    const subject = isString(body.subject)
      ? body.subject.trim()
      : "";
    ...
    if (error) {
      ...
      console.error("Supabase insert error (contact):", {
